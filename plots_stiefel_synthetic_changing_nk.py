...
outer_iter = 600
total_data = 10000
n_list = [50, 100, 500, 1000]  # number of clients
eta_list0 = [1e-4, 2e-4, 3e-4, 4e-4]
eta_list1 = [1e-4, 2e-4, 3e-4, 4e-4]
eta_list2 = [1e-2, 1e-2, 1e-2, 1e-2]
for i in range(4):
    n = n_list[i]
    p = int(total_data / n)
    K = int(n * 0.1)
    ...
    # plt.show()
    plt.savefig('changing_nk/kpca_function_val_' + str(n) + '_' + str(K) + '.pdf')
    plt.clf()
    ...
    # plt.show()
    plt.savefig('changing_nk/kpca_grad_norm_' + str(n) + '_' + str(K) + '.pdf')
    plt.clf()
    ...
    plt.legend(["RFedavg", "RFedprox", "RFedSVRG"])
    # plt.show()
    plt.savefig('changing_nk/kpca_angle_' + str(n) + '_' + str(K) + '.pdf')
    plt.clf()